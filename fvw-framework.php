<?php
  ...
  add_action( 'wp_enqueue_scripts', function() {
    ...
    // Default styles
    wp_enqueue_style( 'fvw-framework-style', plugins_url( '/assets/styles/all.css', __FILE__ ), array( 'flatpickr' ) );
    ...
    // Addon scripts
    if( apply_filters( 'fvw_load_jquery', true ) ) wp_deregister_script( 'jquery' );
    if( apply_filters( 'fvw_load_jquery', true ) ) wp_enqueue_script( 'jquery', plugins_url( '/assets/scripts/jquery.min.js', __FILE__ ), array(), '3.3.1' );
    if( apply_filters( 'fvw_load_fontawesome', true ) ) wp_enqueue_script( 'fontawesome', 'https://kit.fontawesome.com/2b14856f6a.js', array() );
    wp_enqueue_script( 'lazysizes', plugins_url( '/assets/scripts/lazysizes.min.js', __FILE__ ), array(), '5.1.1' );
    wp_enqueue_script( 'fvw-framework-script', plugins_url( '/assets/scripts/framework.min.js', __FILE__ ), array( 'jquery' ) );
    if( apply_filters( 'fvw_load_mobilemenu', true ) ) wp_enqueue_script( 'fvw-mobilemenu', plugins_url( '/assets/scripts/mobilemenu.js', __FILE__ ), array( 'jquery', 'fvw-framework-script' ) );

    // Main script
    $dependencies = array( 'jquery', 'fvw-framework-script', 'flatpickr' );
    if( apply_filters( 'fvw_load_mobilemenu', true ) ) $dependencies[] = 'fvw-mobilemenu';
    wp_enqueue_script( 'fvw-framework-functions', plugins_url( '/assets/scripts/functions.js', __FILE__ ), $dependencies );
